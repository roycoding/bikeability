...
def genmap(geojsonfile, mapfile):
    ...
    folmap.geo_json(geo_path=geojsonfile.replace('output/', ''), data_out='data/data.json', data=df,
                 columns=['label', 'paths'],
                 key_on='feature.id',
...
def main():
    if len(sys.argv) < 3:
        print("Try running me with: ./%s BikePaths.kml zoneatlaspagegrid.kml output/data/ABQsquares.json" % sys.argv[0])
        sys.exit(1)
